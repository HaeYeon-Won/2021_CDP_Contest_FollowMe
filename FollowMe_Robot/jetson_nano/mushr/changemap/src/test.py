...
def select_map(data):
    ...
    if flag == 0 and once_goal == True:
        ...
        print("count = ", count)
        # general execution
        if (count >= 0 and count < len(STATION[state]) and state == start and SUBWAY_IN != subway) or (count >= 0 and count < (len (STATION[state])) and state == end and SUBWAY_IN != subway):
...
if __name__ == "__main__":
    ...
    SUBWAY_IN=" "
    pub_usb_tts=rospy.Publisher("Pub_USB_TTS", Int32, queue_size = 2)
    ...
    if ser1_data == "c":
        ...
        ser_tts = ser1
        pub_usb_tts.publsih(1)
        print ("1 is tts")
    else:
        ser_tf = ser1
        pub_usb_tf.publish(1)
        print ("1 is tf")
    ...
    if ser2_data == "c":
        ...
        ser_vesc = ser2
        pub_usb_vesc.publish(2)
        print ("2 is vesc")
    elif ser2_data == "t":
        ser_tts = ser2
        print ("2 is tts") 
        pub_usb_tts.publish(2)
    else:
        ser_tf = ser2
    ...
    if ser3_data == "c":
        ...
        ser_vesc = ser3
        pub_usb_vesc.publish(3)
        print ("3 is vesc")
    elif ser3_data == "t":
        ser_tts = ser3
        pub_usb_tts.publish(3)
        print ("3 is tts")
    else:
        ser_tf = ser3
        pub_usb_tf.publish(3)
        print ("3 is tf")
    ...
    if ser4_data == "c":
        ...
        ser_tts = ser4
        pub_usb_tts.publish(4)
        print ("4 is tts")
    else:
        ser_tf = ser4
        pub_usb_tf.publish(4)
        print ("4 is tf")
    rate = rospy.Rate(50)
    while True:
        try:
