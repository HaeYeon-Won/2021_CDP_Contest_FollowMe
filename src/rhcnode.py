...
class RHCNode(rhcbase.RHCBase):
    ...
    def setup_pub_sub(self):
        ...
        rospy.Service("goal_set", GoalSet, self.goalSet)
        rospy.Subscriber(
    ...
    def goalSet(self, data):
        global set_state
       
        while(True):
            if set_state==1:
                set_state=0
                return GoalSetResponse("Goal set")
            elif set_state==-1:
                set_state=0
                return GoalSetResponse("Goal fail")
            time.sleep (0.1)

    def cb_goal(self, msg):
        global pub_pi, count, set_state
